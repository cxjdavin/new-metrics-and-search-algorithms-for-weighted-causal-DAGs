...
def compute_minimum_weight_vertex_cover_on_tree(G, w):
    # Root arbitrarily
    root = list(G.nodes)[0]
    dfs_tree = nx.dfs_tree(G, source=root)

    # Run dynamic programming
    memo = dict()
    def dp(v):
        if v not in memo:
            if len(list(dfs_tree.successors(v))) == 0:
                # v is a leaf
                include_v_set, include_v_weight, exclude_v_set, exclude_v_weight = set([v]), w[v], set(), float('inf')
            else:
                # v is not leaf
                include_v_set, include_v_weight = set([v]), w[v]
                exclude_v_set, exclude_v_weight = set(), 0
                for u in dfs_tree.successors(v):
                    include_u_set, include_u_weight, exclude_u_set, exclude_u_weight = dp(u)

                    # If we included v, we can choose to include or exclude u. Pick the best
                    if include_u_weight < exclude_u_weight:
                        include_v_set.update(include_u_set)
                        include_v_weight += include_u_weight
                    else:
                        include_v_set.update(exclude_u_set)
                        include_v_weight += exclude_u_weight

                    # If we excluded v, then we HAVE to include u
                    exclude_v_set.update(include_u_set)
                    exclude_v_weight += include_u_weight

            # Store
            memo[v] = (include_v_set, include_v_weight, exclude_v_set, exclude_v_weight)
        return memo[v]

    include_root_set, include_root_weight, exclude_root_set, exclude_root_weight = dp(root)
    if include_root_weight < exclude_root_weight:
        return include_root_set
    else:
        return exclude_root_set

def compute_weighted_minimum_vertex_cover(H, w):
    assert bipartite.is_bipartite(H)
    mvc = set()
    for V in nx.connected_components(H):
        cc = H.subgraph(V)
        assert nx.is_tree(cc)
        mvc_for_cc = compute_minimum_weight_vertex_cover_on_tree(cc, w)
        mvc.update(mvc_for_cc)
    assert is_vertex_cover(H, mvc)
    return mvc
...
'''
def atomic_verification(G, w = None):
    ...
    if w is None:
        ...
        # Compute weighted minimum vertex cover on H
        mvc = compute_weighted_minimum_vertex_cover(H, w)
